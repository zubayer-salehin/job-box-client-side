...
import { useChatInsertMutation, useGetChatByJobIdQuery } from '../../features/job/jobApi';
            ...
            <div className='w-full h-full flex justify-center'>
                <div className="w-1/2 h-fit mt-5 p-10 border shadow-lg rounded-xl">
                    <div className='flex justify-between'>
                        <h2 className='text-xl font-medium text-primary capitalize mb-5'>Chat With Employer</h2>
                                            ...
                                            <div className='flex flex-col gap-0.5 mr-[25%]'>
                                                {message?.map((m) =>
                                                    <div className='flex justify-start items-end'>
                                                        <p className='bg-gray-500 text-white px-3 py-1.5 rounded-full uppercase mr-1 h-fit'>{role.slice(0, 1)}</p>
                                            ...
                                            <div className='flex flex-col gap-0.5 ml-[25%]'>
                                                {message?.map((m) =>
                                                    <div className='flex justify-end items-end'>
                                                        ...
                                                        <p className='bg-primary text-white px-2.5 rounded-full uppercase py-1.5 h-fit'>{role.slice(0, 1)}</p>
                                                    </div>
                                                )}
                                            </div>}
                                    </div>
