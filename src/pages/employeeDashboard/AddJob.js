...
const AddJob = () => {
  const { user: { companyName, _id } } = useSelector(state => state.auth)
  const [postJob] = usePostJobMutation();
  const { handleSubmit, register, control, reset } = useForm({ defaultValues: { companyName } });
  ...
  const onSubmit = (data) => {
    postJob({ ...data, userId: _id, applicants: [], queries: [] })
    reset()
  };
