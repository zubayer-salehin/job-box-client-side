...
const JobDetails = () => {
  ...
  const handleApply = () => {
    if (user.role === "employer") {
      toast.error("You need to candidate account to apply")
      return;
    }
    if (user.role === "") {
      navigate("/register")
    ...
    apply(data);
    toast.success("Your Apply Was Done")
  }

  const handleQuestion = (data) => {
    const queData = { ...data, questionId: time.getTime(), email: user.email, jobId: _id }
    question(queData);
    reset();
  }
        ...
        <div>
          <div>
            ...
            <div className='text-primary my-2'>
              {queries?.map(({ question, email, reply, questionId }) => (
                <div>
                  <small>{email}</small>
                  ...
                  {user?.role === "employer" && <div className='flex gap-3 my-5'>
                    <input onBlur={(e) => setQuestionReply(e.target.value)} placeholder='Reply' type='text' className='w-full' />
                    <button
